...
public class AzureRedisConfiguration {
    ...
    public RedisClient redisClient(@Value("${redis.uri}") String redisUri,
        ...
        try {
                    ...
                    return Mono.defer(() ->
                        credential.getToken(tokenContext)
                            ...
                            .onErrorMap(e -> new RuntimeException(
                                "Failed to obtain Azure access token for Redis authentication. " +
                                "Ensure the Managed Identity has 'Redis Cache Contributor' or appropriate role assigned.", e))
                    );
                }
            };
            ...
            this.redisClient = RedisClient.create(authenticatedUri);
